    ...
    def play(self, player: int, move: str) -> bool:
        ...
        if self.phase == Shopping.DEFAULT:
            ...
            if transact_match is not None:
                ...
                item_info = next(
                    (item for item in self.item_infos if item_alias == item.alias), None
                )
            ...
            if re.match(
                r"^(그럴래|그래|그랭|넹|네|예|넵|넴|넷|ㅇ{1,4}|응{1,2}|웅{1,2}|우웅|살래|살랭|살거야|삼|산다|살{게}|사고싶어|{줘})$",
                move,
            ):
                # positive response
                ...
                r"^(안그럴래|안그래|안그랭|아니|ㄴ{1,4}|노{1,4}|안살래|안살랭|안살거야|안삼|안산다|안살{게}|안사고싶어|사고싶지않아|주지마)$",
                move,
            ):
                # negative response
                self.message_dict = {
            ...
            if re.match(
                r"^(그럴래|그래|그랭|넹|네|예|넵|넴|넷|ㅇ{1,4}|응{1,2}|웅{1,2}|우웅|팔래|팔랭|팔거야|팜|팖|판다|팔{게}|팔고싶어)$",
                move,
            ):
                # positive response
                ...
                r"^(안그럴래|안그래|안그랭|아니|ㄴ{1,4}|노{1,4}|안팔래|안팔랭|안팔거야|안팖|안팜|안판다|안팔{게}|안팔고싶어|팔고싶지않아)$",
                move,
            ):
                # negative response
                self.message_dict = {
